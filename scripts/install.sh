...
set -euo pipefail
# Keep this file ASCII-only and always write variables as ${VAR}: macOS's bash 3.2
# can treat a non-ASCII byte right after $VAR as part of the name ("DEST?: unbound variable").
...
# Stop a running copy (an AppleScript "quit" could launch it if it isn't running).
if pkill -f "/${APP}/Contents/" 2>/dev/null; then sleep 1; fi
rm -rf "${DEST:?}/${APP}"
ditto "${SRC}" "${DEST}/${APP}"

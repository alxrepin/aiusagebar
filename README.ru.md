...
- **Свой цвет колец для каждого провайдера.** По умолчанию монохром: иконка подстраивается под строку меню или панель задач, как системные. Можно выбрать готовый цвет (красный, зелёный, бирюзовый, как в Apple Fitness, и другие) или любой свой. Цвет применяется к иконке в трее, кольцам и полоскам прогресса.
...
- **Подсказка на Windows:** наведите курсор на иконку в трее, чтобы увидеть проценты и время до сброса по каждому аккаунту.
...
- **Фоновое обновление** по умолчанию раз в минуту (настраивается от 1 до 30 минут). При открытии окна свежие данные подтягиваются сразу. Если API ответил ошибкой или «слишком много запросов», следующие попытки идут всё реже.
...
- **Автообновление.** Приложение каждый час проверяет GitHub Releases в фоне. Когда выходит новая версия, приходит уведомление (раз в сутки, пока не обновитесь) и появляется кнопка «Обновить», которая перезапускает приложение уже в новой версии.
...
- **Русский и английский, светлая и тёмная тема.** По умолчанию следуют системе, в настройках можно выбрать вручную.
...
- **Цвет колец:** по умолчанию монохром, готовый или свой цвет для каждого подключённого провайдера;
...
- **Язык:** системный, English или русский (он же и для уведомлений);
